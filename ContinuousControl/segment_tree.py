...
## class implementing the segment tree data structure
class SegmentTree:
    ...
    def get_interval_sum(self, intv_start, intv_end, current_node,subtree_start, subtree_end):
        """ method that computes the sum of the priority values stored between the intervalas
        """
    ...
    def interval_sum(self, start=0, end=None):
        """method to compute the interval sum
        """
        if end == None:
            end = self.leaf_nodes
    ...
    def find_prefixsum_idx(self, prefixsum):
        """ find the node for which the sum of priority values till
        that node is at least as given sum"""
